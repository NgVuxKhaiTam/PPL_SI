...
def divide_and_conquer(X, XK, a, b, Mobs, Oobs, Lobs, n, nK, p, lambda_sh, lambda_K, rho, z_min, z_max, num_segments=4):
...
def divide_and_conquer_dtf(X_tilde, XK, a, b, c, d, Mobs, Oobs, Lobs, SMobs, SOobs, SLobs, q_tilde, lambda_tilde, P, n, nK, K, z_min, z_max, num_segments=16):
    seg_w = (z_max - z_min) / num_segments
    segments = [(z_min + i * seg_w, z_min + (i + 1) * seg_w) for i in range(num_segments)]
    n_jobs = min(num_segments, os.cpu_count())
